...
$("#home-f-loved li").on("mouseleave",function(c){$(this).find(".color").stop(!0,!0).fadeOut(200);$(this).find(".bw").stop(!0,!0).fadeIn(200)});$("#home-f-loved li").on("mouseenter",function(c){$(this).find(".bw").stop(!0,!0).fadeOut(200);$(this).find(".color").stop(!0,!0).fadeIn(200)});
...
$("#check-nosells").change(function(c){c=$(this).is(":checked");var b=$("input.stepper");c?($("button#invoice").text("Close Suitcase"),$(b).each(function(b,a){$(a).val("0").trigger("change")})):($("button#invoice").text("Request Invoice"),$(b).each(function(b,a){$(a).val("").trigger("change")}))});$(e).find(".tooltip.disabled").tooltipster({position:"top-right"});$(e).find(".content").on("keypress",".item-price input",function(c){if(String.fromCharCode(c.keyCode).match(/[^0-9]/g))return!1});$(e).find(".content").on("click",
".item-price button.add:not(.disabled)",function(e){var b=$(this).siblings("input"),h=$(this).siblings("span"),a=$(this).siblings("button.edit"),d=$(this).attr("data-id");e=$(b).serialize();var g=$(this),d="/suitcase/update-price/"+d;"undefined"!==typeof env&&(d=env+d);$.ajax({beforeSend:function(){if(""!=$(b).val())$(h).text("$ "+c($(b).val())).show(),$(b).hide(),$(g).hide(),$(a).show();else return!1},data:e,dataType:"json",url:d,success:function(a,b,d){$.isEmptyObject(a)},type:"GET"})});$(e).find(".content").on("click",
".item-price button.edit:not(.disabled)",function(c){c=$(this).siblings("input");var b=$(this).siblings("span"),e=$(this).siblings("button.add"),a=$(this);$(b).hide();$(c).show();$(a).hide();$(e).show()});$(e).find(".content").on("click",".winning-bidders button.add:not(.disabled)",function(c){var b=$(this).parent().parent("tr"),e=$(this).attr("data-id");c=$(b).find("input").serialize();e="/suitcase/update-booking/"+e;"undefined"!==typeof env&&(e=env+e);$(b).find("td.first-name form").validate({errorPlacement:function(a,
b){},rules:{"booking[first]":{required:!0}}});$(b).find("td.last-name form").validate({errorPlacement:function(a,b){},rules:{"booking[last]":{required:!0}}});$(b).find("td.email form").validate({errorPlacement:function(a,b){},rules:{"booking[email]":{required:!0,email:!0}}});$(b).find("td.phone form").validate({errorPlacement:function(a,b){},rules:{"booking[phone]":{required:!0}}});var a=$(b).find("td.first-name input").valid(),d=$(b).find("td.last-name input").valid(),g=$(b).find("td.email input").valid(),
k=$(b).find("td.phone input").valid();if(!(a&&d&&g&&k))return!1;$.ajax({beforeSend:function(){var a=$(b).find("td.first-name input").val(),d=$(b).find("td.last-name input").val(),c=$(b).find("td.email input").val(),e=$(b).find("td.phone input").val();$(b).find("td.first-name span").text(a).show();$(b).find("td.last-name span").text(d).show();$(b).find("td.email span").text(c).show();$(b).find("td.phone span").text(e).show();$(b).find("td.first-name input").hide();$(b).find("td.last-name input").hide();
$(b).find("td.email input").hide();$(b).find("td.phone input").hide();$(b).find("td.actions > button.add").remove();$(b).find("td.actions > button.edit").show();$(b).find("td.actions > button.mail").show()},data:c,dataType:"json",url:e,success:function(a,b,d){$.isEmptyObject(a)},type:"POST"})});$(e).find(".content").on("click",".winning-bidders button.edit:not(.disabled)",function(c){var b=$(this).parent().parent("tr"),e=$(this).attr("data-id");c=$(b).find("input").serialize();e="/suitcase/update-booking/"+
e;"undefined"!==typeof env&&(e=env+e);var a=$(b).find("td.first-name"),d=$(b).find("td.last-name"),g=$(b).find("td.email"),k=$(b).find("td.phone"),m=$(this);if($(this).hasClass("neutral"))$(a).find("input").show(),$(a).find("span").hide(),$(d).find("input").show(),$(d).find("span").hide(),$(g).find("input").show(),$(g).find("span").hide(),$(k).find("input").show(),$(k).find("span").hide(),$(m).removeClass("neutral");else{$(b).find("td.first-name form").validate({errorPlacement:function(a,b){},rules:{"booking[first]":{required:!0}}});
$(b).find("td.last-name form").validate({errorPlacement:function(a,b){},rules:{"booking[last]":{required:!0}}});$(b).find("td.email form").validate({errorPlacement:function(a,b){},rules:{"booking[email]":{required:!0,email:!0}}});$(b).find("td.phone form").validate({errorPlacement:function(a,b){},rules:{"booking[phone]":{required:!0}}});a=$(b).find("td.first-name input").valid();d=$(b).find("td.last-name input").valid();g=$(b).find("td.email input").valid();k=$(b).find("td.phone input").valid();if(!(a&&
d&&g&&k))return!1;$.ajax({beforeSend:function(){var a=$(b).find("td.first-name input").val(),d=$(b).find("td.last-name input").val(),c=$(b).find("td.email input").val(),e=$(b).find("td.phone input").val();$(b).find("td.first-name span").text(a).show();$(b).find("td.last-name span").text(d).show();$(b).find("td.email span").text(c).show();$(b).find("td.phone span").text(e).show();$(b).find("td.first-name input").hide();$(b).find("td.last-name input").hide();$(b).find("td.email input").hide();$(b).find("td.phone input").hide();
$(m).addClass("neutral")},data:c,dataType:"json",url:e,success:function(a,b,d){$.isEmptyObject(a)},type:"POST"})}});$(e).find(".content").on("click",".winning-bidders button.mail:not(.disabled)",function(c){var b=$(this).parent().parent("tr");c=$(this).attr("data-id");var e=$(b).find("td.first-name input").val(),a=$(b).find("td.last-name input").val(),d=$(b).find("td.email input").val();$(b).find("td.phone input").val();var g=$(b).attr("data-code"),k=$("#test").attr("data-np"),m=$("#test").attr("data-first"),
p=$("#test").attr("data-last"),q=$("#test").attr("data-phone"),r=$("#test").attr("data-email"),s=$("#test").attr("data-event"),t=$("#test").attr("data-date"),n=$(b).parent().parent().parent().parent().parent("li"),b=$(n).attr("data-header"),u=$(n).attr("data-name"),n=$(n).attr("data-code"),l;l="Dear "+e+",\n\nCongratulations on your winning bid and thank you so much for your contribution! We appreciate you coming out and supporting our organization, and hope you had a wonderful time at our fundraising event. From all of us here at ";
...
$.ajax({beforeSend:function(){var b=$("#mail-modal #voucher_id").val();$('button.edit[data-id="'+b+'"]').removeClass("neutral").addClass("disabled").attr("title","You\u2019ve already sent this voucher.<br/>Contact your EC to change").tooltipster({position:"top-right"});$.modal.close();$("#mail-thanks-modal").find(".number span").text("1");$("#mail-thanks-modal").find(".name").text($("#mail-modal #mail-fullname").text());$("#mail-thanks-modal").find(".email").text($("#mail-modal #mail-to").text());
$("#mail-thanks-modal").modal({closeText:"X",overlay:"#fff",opacity:0.73,zIndex:2002})},data:b,dataType:"json",url:c,success:function(b,a,d){$.isEmptyObject(b)},type:"POST"})});$("#mail-thanks-modal").find("button").on("click",function(c){c.preventDefault();$.modal.close()});$("#mail-modal").on($.modal.OPEN,function(c,b){$(window).width()<$(document).width()&&$(b.elm).css({marginLeft:Math.floor(($(document).width()-$(b.elm).outerWidth())/2)+"px",left:"0"});94>$(window).height()-$(b.elm).outerHeight()&&
($(b.elm).css({position:"absolute",marginTop:"120px",top:"0"}),$(window).scrollTop(100))})});
