...
//            define('WP_USE_THEMES', false);
    ...
    protected function getSuitcase()
    {
        ...
        if($this->get('security.context')->isGranted('ROLE_ADMIN')) {
            if($sid) {
                $query = $em->createQuery(
                    'SELECT s, i FROM InertiaWinspireBundle:Suitcase s LEFT JOIN s.items i WHERE s.id = :id AND (i.status != \'X\' OR i.status IS NULL) ORDER BY i.updated DESC'
                )
                ->setParameter('id', $sid)
                ;
...
//        $sid = $session->get('sid');
        if($sid) {
            ...
            $query = $em->createQuery(
                'SELECT s, i, p FROM InertiaWinspireBundle:Suitcase s LEFT JOIN s.items i LEFT JOIN i.package p WHERE s.user = :user_id AND s.id = :id AND (i.status != \'X\' OR i.status IS NULL) ORDER BY i.updated DESC'
            )
        ...
        // Second, query for the most recent suitcase (used as default)
            ...
            $query = $em->createQuery(
                'SELECT s, i FROM InertiaWinspireBundle:Suitcase s LEFT JOIN s.items i LEFT JOIN i.package p WHERE s.user = :user_id AND (i.status != \'X\' OR i.status IS NULL) ORDER BY s.updated DESC, i.updated DESC'
            )->setParameter('user_id', $user->getId());
