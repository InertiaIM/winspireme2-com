...
//echo 'Found SID, step 1: ' . $sid . "<br/>\n";
            $query = $em->createQuery(
                'SELECT s, i FROM InertiaWinspireBundle:Suitcase s JOIN s.items i WHERE s.user = :user_id AND s.id = :id ORDER BY i.updated DESC'
            )
...
//                throw $this->createNotFoundException();
        ...
        // Second, query for the most recent suitcase (used as default)
            ...
            $query = $em->createQuery(
                'SELECT s, i FROM InertiaWinspireBundle:Suitcase s JOIN s.items i WHERE s.user = :user_id ORDER BY s.updated DESC, i.updated DESC'
            )->setParameter('user_id', $user->getId());
